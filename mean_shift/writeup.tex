...
We first evaluated baselines using global processing (ignoring plate boundaries):
\begin{itemize}
    ...
    \item \textbf{Cross-cell-line approach}: DMSO-corrected (later termed V1 in Stage 2)
    \item \textbf{Baselines}: Control Passthrough, Mean Shift Ablation, Nearest-Cell-Line, Cross-Cell-Line (DMSO-corrected), State Model
\end{itemize}
...
Cross-Cell-Line (DMSO-corrected) & 0.061 & 0.010 \\
...
\begin{itemize}
    \item Mean shift ablation (oracle) establishes an upper bound on achievable performance in both few-shot and zero-shot
    \item State model competitive with oracle in few-shot setting (0.032 vs 0.036 RBF MMD)
    \item Zero-shot generalization significantly more challenging: State model dramatically underperforms (0.669 RBF MMD) while simple baselines remain effective
    \item Scale correction ($\div 40$) critical for zero-shot evaluation
    \item Need for batch-aware processing and additional baseline variants motivated Stage 2 experiments
\end{itemize}
...
Additionally, we introduced a second cross-cell-line variant (V2) to investigate whether using the test cell line's DMSO mean instead of correcting each training cell line's baseline would affect performance. This allowed us to compare the original DMSO-corrected approach (V1) against an alternative that computes shifts relative to the target cell line's baseline (V2).

\subsubsection{Experimental Setup}

\begin{itemize}
    \item \textbf{Per-plate processing}: All operations restricted to within-plate data
    \item \textbf{Cross-cell-line variants}: V1 (DMSO-corrected, from Stage 1) and V2 (test mean, newly introduced)
