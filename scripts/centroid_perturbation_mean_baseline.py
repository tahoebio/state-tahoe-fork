...
def evaluate_batch(batch_adata, batch_name, explicit_splits, holdout_cells, zeroshot_cells, control_pert, pert_col,
    ...
    perturbations = [p for p in batch_adata.obs[pert_col].unique() if p != actual_control]
    ...
    for pert in tqdm(perturbations, 
                     ...
                     leave=False):
        training_deltas = []
        for idx in batch_adata.obs[batch_adata.obs[pert_col] == pert].index:
            cell_line = batch_adata.obs.loc[idx, cell_col]
    ...
    for pert in tqdm(all_perturbations, desc="  Computing global training effects", leave=False):
        ...
        for data_point in all_data:
            if data_point['perturbation'] == pert:
                cell_line = data_point['cell_line']
                # Check if this is training data using same logic as batch-specific version
                is_training = (
                    cell_line not in zeroshot_cells and  # Exclude ALL data from zeroshot cells
                    (cell_line not in holdout_cells or 
                     explicit_splits.get((cell_line, pert)) is None)
...
def main():
    ...
    output_file = f"{args.output_dir}/perturbation_mean{abs_suffix}_detailed_correlations.csv"
    full_df.to_csv(output_file, index=False)
    print(f"Saved detailed correlations to: {output_file}")

    # Save summaries as JSON
    summary_file = f"{args.output_dir}/perturbation_mean{abs_suffix}_hierarchical_summaries.json"
