...
def estimate_plate_memory(state_path: str, mosaicfm_path: str, plate_value: str) -> Tuple[int, float]:
    """Estimate memory usage for a specific plate."""
    with StringCache():
        state_lf = pl.scan_parquet(state_path)
        plate_rows = state_lf.filter(pl.col('plate') == plate_value).select(pl.len()).collect().item()
        
    # Estimate memory: rows × (2048 + 512 + ~20000 HVG genes) × 4 bytes/float32
    # Plus overhead for obs data
    memory_gb = plate_rows * (2048 + 512 + 20000) * 4 / (1024**3)
...
def process_plate_chunked(state_lf, mosaicfm_lf, plate_value: str, token_to_col_idx: Dict[int, int], 
    ...
    while current_row < plate_rows:
        ...
        # Process chunk into temporary arrays
        hvg_chunk = np.zeros((actual_chunk_size, n_hvg_genes), dtype=np.float32)
        mosaicfm_chunk = np.zeros((actual_chunk_size, 512), dtype=np.float32)
        state_chunk = np.zeros((actual_chunk_size, 2048), dtype=np.float32)
        ...
        for i, (s_row, m_row) in enumerate(zip(
            ...
            # Process gene expression to HVG
            genes = s_row['genes']
            ...
            # Assign to chunk arrays
            hvg_chunk[i] = hvg_vec
            mosaicfm_chunk[i] = np.array(m_row['mosaicfm-70m-merged'], dtype=np.float32)
            state_chunk[i] = np.array(s_row['state_embeddings'], dtype=np.float32)
            all_obs_data[current_row + i] = obs_row
        
        # Copy chunk data directly to final arrays (no accumulation)
        # Ensure chunk arrays have correct size before assignment
...
def process_plate_whole(state_lf, mosaicfm_lf, plate_value: str, token_to_col_idx: Dict[int, int],
    ...
    # Initialize matrices
    hvg_matrix = np.zeros((plate_rows, n_hvg_genes), dtype=np.float32)
    mosaicfm_matrix = np.zeros((plate_rows, 512), dtype=np.float32)
    state_matrix = np.zeros((plate_rows, 2048), dtype=np.float32)
    obs_data = []
    
    # Process with progress bar
    plate_pbar = tqdm(total=plate_rows, desc=f"Plate {plate_value}", unit="cells", leave=False)
    ...
    for i, (s_row, m_row) in enumerate(zip(
        ...
        assert s_row['BARCODE_SUB_LIB_ID'] == m_row['BARCODE_SUB_LIB_ID'], \
            f"Data alignment mismatch: {s_row['BARCODE_SUB_LIB_ID']} != {m_row['BARCODE_SUB_LIB_ID']}"
        ...
        if (i + 1) % 10000 == 0:
            plate_pbar.update(10000)
    
    # Update progress bar for remaining cells
    remaining = plate_rows % 10000
    if remaining > 0:
        plate_pbar.update(remaining)
...
def main(cfg: DictConfig):
    ...
    with StringCache():
        ...
        for plate_idx, plate_value in enumerate(plates):
            ...
            plate_rows, memory_gb = estimate_plate_memory(cfg.state_path, cfg.mosaicfm_path, plate_value)
